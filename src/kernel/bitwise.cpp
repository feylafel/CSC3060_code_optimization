...
// Using (~shared & ~kMaskLo) = (~shared) & (~kMaskLo); use andnot(shared,
// not_mlo) with not_mlo = mlo^0xFF per byte.
namespace {
#if defined(__AVX2__)
inline __m256i bitwise_lane_avx2(__m256i va, __m256i vb, __m256i mlo,
                                 __m256i mhi, __m256i not_mlo,
                                 __m256i not_mhi) {
    ...
    const __m256i mixed0 =
        _mm256_or_si256(_mm256_and_si256(diff, mlo),
                        _mm256_andnot_si256(shared, not_mlo));
    const __m256i mixed1 =
        _mm256_xor_si256(_mm256_and_si256(_mm256_xor_si256(either, mhi),
                                          _mm256_or_si256(shared, not_mhi)),
                         diff);
    return _mm256_xor_si256(mixed0, mixed1);
}
#endif
inline __m128i bitwise_lane_sse(__m128i va, __m128i vb, __m128i mlo,
                                __m128i mhi, __m128i not_mlo, __m128i not_mhi) {
