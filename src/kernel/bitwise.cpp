...
// Uaing (~shared & ~kMaskLo) = (~shared) & (~kMaskLo); use andnot(shared, not_mlo) with not_mlo = mlo^0xFF per byte.
namespace {
#if defined(__AVX2__)
    ...
    inline __m128i bitwise_lane_sse(__m128i va, __m128i vb, __m128i mlo, __m128i mhi,
        ...
        const __m128i mixed0 = _mm_or_si128(
            _mm_and_si128(diff, mlo), _mm_andnot_si128(shared, not_mlo));
        const __m128i mixed1 = _mm_xor_si128(
            _mm_and_si128(_mm_xor_si128(either, mhi),
...
void stu_bitwise(std::span<std::int8_t> result,
                   ...
                   std::span<const std::int8_t> b)  {constexpr std::uint8_t kMaskLo = 0x5Au;
                    ...
                    for (; i < n64; i += 64) {
                        _mm_prefetch(reinterpret_cast<const char*>(pa + i + 128), _MM_HINT_T0);
                        _mm_prefetch(reinterpret_cast<const char*>(pb + i + 128), _MM_HINT_T0);
                        const __m256i va0 =
                    ...
                    for (; i < n16; i += 16) {
                        ...
                        const __m128i outv = bitwise_lane_sse(va, vb, mlo128, mhi128, not_mlo128,
                                                              not_mhi128);
